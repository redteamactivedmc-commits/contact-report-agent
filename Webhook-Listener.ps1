...
# ── Helper: save report as .docx via Word COM ─────────────────────────────────
function Save-Report {
    ...
    $filename  = "ADMC_ContactReport_${safeName}_${datestamp}.docx"
    ...
    try {
        ...
        foreach ($line in $ReportText -split "`n") {
        ...
        $doc.SaveAs([ref]$fullPath, [ref]12)
        $doc.Close($false)
    }
    finally {
        $word.Quit()
